...
import { Atom, Brain, ChartLine, ChartLineUp, Database, FileText, Fire, Flask, Gauge, Gavel, Gear, Graph, HardDrives, ListBullets, ListMagnifyingGlass, Pulse, Question, ShieldCheck, ShieldStar, SlidersHorizontal, Sparkle, Strategy } from "@phosphor-icons/react";

export type VNextModuleGroup = "research" | "trading" | "control";
export interface VNextModule { id:string; path:string; label:string; caption:string; group:VNextModuleGroup; icon:Icon; keywords:string; }
export const vnextModuleGroups:Array<{id:VNextModuleGroup;label:string}>=[{id:"research",label:"RESEARCH"},{id:"trading",label:"TRADING"},{id:"control",label:"CONTROL"}];
...
export function moduleForVNextPath(path:string):VNextModule{const target=new URL(path,window.location.origin);const candidates=vnextModules.map(module=>{const registered=new URL(module.path,window.location.origin);if(registered.pathname!==target.pathname)return null;const required=[...registered.searchParams.entries()];if(required.some(([k,v])=>target.searchParams.get(k)!==v))return null;return{module,score:required.length}}).filter((item):item is {module:VNextModule;score:number}=>item!==null).sort((a,b)=>b.score-a.score);return candidates[0]?.module??vnextModules[0];}
export function contextForPath(path:string){const url=new URL(path,window.location.origin);const read=(key:string):string|undefined=>url.searchParams.get(key)??undefined;return{symbol:read("symbol"),experiment:read("experiment"),run:read("run")??read("runId"),model:read("model")??read("modelId"),portfolio:read("portfolio"),account:read("account"),artifact:read("artifact")??read("query")};}
