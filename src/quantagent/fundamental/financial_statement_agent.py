    ...
    for _, row in data.iterrows():
        ...
        rows.append(
            FundamentalScore(
                ...
                confidence=float(np.clip(0.35 + quality / 300.0 + earnings_visibility / 300.0 - fraud / 400.0, 0.05, 0.95)),
                rationale=_rationale(row, quality, growth, valuation, fraud),
...
def _rationale(row: pd.Series, quality: float, growth: float, valuation: float, fraud: float) -> str:
    return (
        f"quality={quality:.1f}, growth={growth:.1f}, valuation={valuation:.1f}, "
        f"fraud_risk={fraud:.1f}, theme_exposure={_safe_float(row.get('theme_revenue_exposure', 0.0)):.1f}"
    )
