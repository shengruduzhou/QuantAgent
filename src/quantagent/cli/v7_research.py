"""Legacy research CLI: factor / flow / sector-rotation / valuation reports.

These commands are preserved so the existing entry points (``quantagent-build-factors``,
``quantagent-build-flow-features`` etc.) and tests keep working. They sit
alongside the V7 real-data CLI commands rather than replacing them.
"""
