...
| 04 Limit-up ladder | `/market-intelligence` → 市场脉冲 | limit-up pool + 30-session ladder; pool fields keep limit-up time/reason, continuation count and seal amount |
| 05 Watchlist anomalies | `/market-intelligence` → 市场脉冲 | documented REST anomaly list is native; future watchlist filtering must only filter these returned events |
...
| 12 Limit-up sentiment | `/market-intelligence` → 市场脉冲 | pool counts and ladder are finite returned samples; do not infer unsupported market-wide promotion rates |
...
6. Market observation and QuantAgent decision evidence remain separate layers. Hot lists, limit-up data, anomaly interpretations and dragon-tiger data are not direct buy/sell signals.
