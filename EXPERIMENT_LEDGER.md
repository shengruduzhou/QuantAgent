...
## H-031 · 2026-07-23 · VNext 集成 / 影子证书闭合 / 全宇宙数据就绪（**运营+数据票，非研究——零 alpha 研究/模型选择/因子挖掘/回测排名/表现解盲**）

- **Track I（分支血缘）**：H-030 四提交（6d23735/e42e478/5a10b5c/c63ee06）**已线性合入 main 且在 origin/main（可远程恢复）**；VNext + DataManager 工作叠加其上（177 提交），**非分叉**。重叠文件仅 1（tests/test_h030_operational_gates.py，被 35229a8 CI 提交**加强**非削弱——加了 fixture 化 supersession 覆盖）。三个 H-030 后端脚本与 H-030 blob **逐字节相同**。预期冲突区 = **0**；真实"集成"缺口 = 架构（脚本未注册为受治理命令、无 VNext 面）。产物：runtime/reports/h031/{branch_lineage.json, integration_plan.md}。集成分支 `agent/h031-vnext-integration`。
- **Track F（影子闭合）**：从 append-only ledger 重建权威影子注册表（非文件名），硬化 validator（新增：解密/提前解盲全局关卡、schema-hash 跨日稳定关卡、partial-intraday-bar 显式原因、accumulating 状态器 + 下一预期有效日）。**判定 = SHADOW_TEST_ACCUMULATING，2/7 有效日**（2026-07-21、2026-07-22）；排除 07-15/16（全失败）、07-17/20（data_status=FAILED，T-1 top-up backlog）。**未提前签证书**（正确）。13/13 gate 测试通过。
- **Track S4（批量重放保全）**：重跑 s4_batch_replay.py，**S4_BATCH_REPLAY_READY 复认**（exact/deterministic/archived 全真，26 cutoff、0 mismatch）；冻结学习器与存储 trace 哈希**未变**；唯一 delta = 前向归档追加增长（sleeve_scores 3.21→3.27MB、panel 502.36→502.43MB，benign，不影响 pre-quarantine 重放）。产物：runtime/reports/h031/s4_reverification.json。
- **Track U0（全宇宙数据就绪）**：新建三脚本（u0_provider_coverage/u0_build_security_master/u0_audit）+ 运行 assemble 合并 157 staged。**决定 = FULL_UNIVERSE_DATA_NOT_READY_COVERAGE**（integration PASS / provider PASS / coverage FAIL / pit FAIL）。覆盖 4,028/5,888；STAR(613)+BSE(327) **零覆盖**（NOT_PROBED 未回填，非 provider 无能）；1,860 BLOCKED_BY_DATA；PIT 字段 ST/停牌/退市日/公司行动 = BLOCKED_BY_DATA（磁盘无 PIT 源，**未伪造**）。产物：runtime/data/u0/{provider_coverage_matrix.parquet+csv(26 列), historical_security_master.parquet, pit_field_availability.json, full_universe_readiness_certificate.json+report.md}。
- **受治理命令**：JobRunner allowlist 新增 7（validate-shadow-days/certify-s4-batch-replay/build-u0-security-master/report-u0-provider-coverage/assemble-u0-full-universe/audit-u0-full-universe/backfill-u0-market-panel）——typed schema、Runtime-only 输出、backfill 显式 --allow-network fail-closed、无自由 shell 字段、fixed_args 子命令支持。
- **VNext 面**：新增 `/governance` 只读页（影子有效日/Track-F 健康/保真哈希/S4 就绪/U0 覆盖+PIT 关卡/缺席板块/血缘/受治理命令），复用 shell + semantic token + WorkbenchHeader/MetricStrip；"全宇宙训练"控制**保持禁用直至 FULL_UNIVERSE_DATA_READY**；后端 GovernanceService 存在性/关卡级字段 + **表现非披露断言（word-boundary，防 "unavailable"→"nav" 假阳）**。
- **测试**：backend 97 通过（quant_ui + h030 13 + h031 governed 26 + u0 artifacts 6）；frontend 36 通过（含 governance 5：禁用门/无 shell 字段/无表现泄漏/不可用态/主题）；tsc --noEmit 干净；vite build 成功（governance chunk 产出）；compileall OK。
- **H-032 预注册草案（未武装）**：runtime/reports/h031/full_universe_training_preregistration_draft.md，候选集恰四（U-B0/U-M0/U-M1/U-M2），**不重开模型园/因子搜索，不复用当前 Track-F FRESH 期**；下一代盲测时钟仅在 U0 就绪+历史验证+候选冻结后启动。
- 资源：RSS ≤3.8G（S4 重放/assemble）；无 GPU；新磁盘 ~0.3MB（u0 matrix/master）+ assemble 重写现有 panel（净增≈0）；网络 = 0（未跑 backfill fetch）。改动文件：scripts/{shadow_day_registry,u0_full_universe_backfill,u0_provider_coverage,u0_build_security_master,u0_audit}.py、services/quant_api/services/{jobs,governance,container}.py、services/quant_api/routes/api.py、apps/quant-ui/src/vnext/governance/*、modules.ts、WorkspaceRoutes.tsx、main.tsx、governance.css、tests/{quant_ui/test_h031_governed,test_h031_u0_artifacts}.py + GovernancePage.test.tsx、runtime/reports/h031/*、runtime/data/u0/*。

## H-032A · 2026-07-23 · 基于 main 的 U0 覆盖与 PIT 就绪闭合（**数据+运营票，非训练**；分支 agent/h032a-u0-readiness，从最新 main 切）

- **主线血缘核验**：H-031 提交 a9e8260 经 PR #19（cd9bcb9）合入，main==origin/main==cd9bcb9；a9e8260 从 main 与 origin/main **均可达**（非 squash）；8 项能力（governed 命令/GovernanceService/2 路由/影子 validator/S4 certifier/U0 master+coverage+assemble+audit）文件与测试等价核验通过（45 测试）。未搜索/恢复/重并旧分支。
- **Track F 连续性**：从 append-only ledger 重建权威影子注册表；**新增 2026-07-23 为第 3 个有效日 → 3/7，SHADOW_TEST_ACCUMULATING**（未提前签证书）；未把失败历史日转成有效日；下一预期有效日 2026-07-24。
- **Track S4**：冻结学习器/certifier/trace 三哈希全长比对**未变** ⇒ 按规则**不重跑**，S4_BATCH_REPLAY_READY 保全。
- **STAR/BSE 代表性探针（§6，26 符号活网探针，rate-limited、yield Track-F、published-close 钳制）**：连通性对照 600000.SH OK。**STAR 8/8 COVERED ⇒ FETCHABLE_NOT_PROBED（纯回填 backlog，非 provider/权限/身份问题）**；**BSE master 920xxx 10/10 COVERED，但已知真实 8xxxxx 代码 0/8 全 EMPTY，且 master 含 0 个 8xxxxx 代码（327 全 920xxx）⇒ BSE 身份完整性 BLOCKED_BY_DATA（无权威代码迁移表，供应商不服务 8xxxxx）**。产物 runtime/data/u0/star_bse_probe_report.json。
- **provider coverage 词汇强化（§5）**：新增 provider_retry_class（OK/FETCHABLE_NOT_PROBED/NO_RELIABLE_HISTORY/NOT_PROBED）+ source_boundary 两列（→28 列）；strict 词汇 EMPTY_PROVIDER_RESPONSE 取代 EMPTY。retry_class 计数：OK 4032 / FETCHABLE_NOT_PROBED 936 / NOT_PROBED 847 / NO_RELIABLE_HISTORY 73（和=5888）。
- **退市生存者偏差（§7）量化**：358 退市（6.1%），226 有行情、**132 无行情、0 有退市日**；退市日/区间 BLOCKED_BY_DATA ⇒ 阻止 READY（除非预注册规则许可有界排除）。
- **PIT 溯源（§8）**：每字段带 available_at/source/source_timestamp/source_hash（可得者）或 BLOCKED_BY_DATA（ST/停牌/退市日/公司行动/代码迁移）；不以现状重构历史、不 default-false。
- **有界后台回填演示（§10）**：governed backfill 加 --priority-boards（STAR/BSE 前置），18min 有界后台运行、yield Track-F、per-symbol 断点，**SIGTERM 优雅取消、锁安全释放、staged 保全**（演示可取消性）；实取回 4 个 STAR 全历史（证明端到端路径），完整板块回填=多夜后台任务。
- **bar-panel 门（§9）**：panel 内容哈希 331c118b…、15,407,050 行、4033 票、2026-07-23 止；重复键/退市后/未发布当日/负价/部分日 全 PASS；**eligible-null-close + suspended-representation FAIL**（绑定 suspension BLOCKED_BY_DATA，全宇宙 panel 缺 is_suspended 旗标）。
- **单一数据决定 = FULL_UNIVERSE_DATA_NOT_READY_COVERAGE**（integration PASS/provider PASS/coverage FAIL/bar_panel FAIL/pit FAIL；training_permitted=False）。新增第 8 governed 命令 probe-u0-star-bse（network 门控）+ priority_boards 参数。UI governance 面扩展（retry 类计数/backlog/生存者偏差/STAR-BSE 探针/回填进度）。
- **测试**：backend 53（h030 13 + h031 governed 34 + u0 artifacts 10，含网络 fail-closed 子进程、priority_boards、bar-panel、生存者、探针词汇）；frontend 36（governance 5 含生存者/探针断言）；tsc 干净；vite build OK；compileall OK；git diff --check 干净。资源：活网探针 ~3min（26 req）+ 有界回填 18min（4 STAR 全历史）；RSS ≤4G；无 GPU；新磁盘 ~数十 MB（4 STAR staged + probe report）。改动文件：scripts/{u0_provider_coverage,u0_build_security_master,u0_audit,u0_full_universe_backfill,u0_star_bse_probe}.py、services/quant_api/services/{jobs,governance}.py、apps/quant-ui/src/vnext/governance/{GovernancePage.tsx,GovernancePage.test.tsx}、tests/{quant_ui/test_h031_governed,test_h031_u0_artifacts}.py。

## H-032B · 2026-07-23 · TickFlow 原生 U0 条数据与 PIT 闭合（**数据+运营票，非训练**；分支 agent/h032b-tickflow-u0-closure，从最新 main）

- **主线核验**：H-032A（1ac7f37）经 PR #21（0a379dd）合入 origin/main，可达；12 项 H-032A 能力（含 8 governed 命令）文件+测试等价核验；从 0a379dd 切分支。
- **§3 TickFlow 能力基准（决定性根因）**：SDK 0.1.22，`api.tickflow.org`，paid。**`klines.get(count=10000)` 一次返回全历史（600000.SH 6352 行/0.4s；跨 Main/ChiNext/STAR/BSE-920 全 OK）；省略 count 恰返回 100 行 = 旧 ~100-bar "server cap" 的真因（缺 count 参数，SDK 默认 100，非服务器截断）**。`klines.batch` = PermissionError（无批量权限）。**实测硬限速 10 req/min**（RateLimitError "请求频率超限 (10/min)"；SDK 静默重试 429 掩盖了首估的 55/min；60/min 是 free-tier 默认）。产物 runtime/reports/h032b/tickflow_capability_{benchmark.json,report.md}。
- **§4 TickFlow 原生回填**：`fetch_full_history` 由 60 页回退循环改为单 `count=10000` 请求 ⇒ **每票 ~6s（10/min 下）vs 旧 ~400s（~64 页）= ~60x 提速**；REQ_INTERVAL 6.5s（10/min 守限）。修 trackf_busy() 假阳（裸子串匹配把监控 grep 当 Track-F → 精确匹配 `<name>.(py|sh)`）；修 priority-boards 尊重显式顺序（BSE 先于 STAR，原按符号字母序把 688<920）。有界后台回填 BSE-first，SIGTERM 优雅取消+锁释放，本会话 staged 54 BSE+24 STAR（全历史，count=10000 路径）。
- **§5/§6 BSE 身份修复（纠正 H-032A 过度分类）**：akshare 权威 BSE 列表 = **329 只全 920xxx、0 个 8xxxxx**；master 327 全在其中（重叠 327，缺 2=920079/920117），**0 个真占位码**。TickFlow instruments.get 识别 920002=万达轴承、830799 返回空 ⇒ **920xxx 是权威当前代码（BSE 已迁移），8xxxxx 是废弃旧码；H-032A "BSE 身份 BLOCKED" 是过度分类**。判定 **BSE_IDENTITY_CURRENT_RESOLVED**；仅老码迁移表 historical_symbol=BLOCKED_BY_DATA（次要，不阻塞条数据/当前身份）。产物 bse_code_mapping.parquet + bse_identity_audit.json。
- **§9 PIT 源审计（活探）**：klines 仅 OHLCV+身份（无 ST/停牌/涨跌停/status 旗标）；instrument.ext 仅当前快照（listing_date/当前 limit/股本，无退市日无区间）；**ex_factors=PermissionError（无除权因子权限）**。分类：ST=ALTERNATIVE_SOURCE_REQUIRED、停牌=TICKFLOW_CURRENT_ONLY、退市=ALTERNATIVE_SOURCE_REQUIRED、涨跌停 regime=TICKFLOW_CURRENT_ONLY、IPO=可派生、公司行动=ALTERNATIVE_SOURCE_REQUIRED。产物 pit_source_audit.json。
- **§8 条数据就绪证书**：4,107 票、BSE 54/STAR 24 现已覆盖、**boards_absent=[]（五板全在列）**、identity PASS、provider PASS、coverage FAIL（backlog）、quality FAIL（null_close/停牌旗标缺）⇒ **U0_BAR_NOT_READY_COVERAGE**。§10 严格 PIT 证书：BSE 身份 RESOLVED、PIT 4 字段 BLOCKED ⇒ **FULL_UNIVERSE_DATA_NOT_READY_PIT**。**§11 训练门 = PIT 决定；未 READY ⇒ 正式训练禁；条数据 READY 仅解锁 smoke（数据集/特征/内存/CLI）**。
- **§14 剩余时长估算（实测 10/min + count=10000）**：剩 ~1,782 票 ÷ 10/min ≈ **~3 小时**有界后台（对比旧分页路径 ~198 小时）。
- **Track F/S4 保全**：影子 3/7 ACCUMULATING（07-23 已计，未提前签证）；S4 冻结学习器哈希未变 ⇒ 不重跑，S4_BATCH_REPLAY_READY 保全。**未改 S1-S4/FRESH 钟/影子注册表/加密表现台账/冻结推理运行时**；未看候选表现。
- **新增 4 governed 命令**（→12 总）：benchmark-tickflow-capability、audit-bse-identity（网络门控）、audit-u0-pit-readiness、report-u0-bar-readiness。VNext governance 面新增 H-032B 分列面板（条 vs 严格 PIT 就绪、TickFlow 基准、BSE 身份、PIT 源审计）。
- **测试**：backend 121（h030 13 + quant_ui 全 + h031 u0 artifacts 10 + h032b 6：两决定分离/基准 count10000/BSE 非占位/PIT 词汇/count-single-request/优先序）；frontend 36（governance 5 含 bar/PIT/BSE 断言）；tsc 干净；vite build OK；compileall OK；git diff --check 干净。资源：活网基准+探针+PIT 探针 ~30 req；有界回填 ~4min×多轮（10/min）；RSS ≤4G；无 GPU；新磁盘 ~数十 MB（BSE/STAR staged + h032b 报告）。改动文件：scripts/{tickflow_capability_benchmark,u0_bse_identity,u0_pit_readiness,u0_bar_readiness,u0_full_universe_backfill}.py、services/quant_api/services/{jobs,governance}.py、apps/quant-ui/src/vnext/governance/{GovernancePage.tsx,GovernancePage.test.tsx}、tests/{quant_ui/test_h031_governed,test_h032b_tickflow_u0}.py。

## H-032C · 2026-07-24 · U0 TickFlow 回填完成与严格 PIT 闭合（**数据+运营票，非训练**；分支 agent/h032c-u0-finalize，从最新 main）

- **主线核验**：H-032B（a1b4178）经 PR #22（9d7a523）合入 origin/main，可达；H-032B 全能力（count=10000/6.5s pacing/BSE 920 身份/两证书/governed backfill/VNext bar-vs-PIT）文件核验通过；从 9d7a523 切分支。
- **Track-F 绝对优先（关键运营约束）**：本会话期间 Track-F **正在跑 catchup_supervisor + catchup_panel_chunked（timeout 10800=3h，会话内一直活跃，1500/3487 staged，正占用共享 TickFlow 10/min 预算）+ coverage_guard**。故所有 TickFlow 作业（回填、entitlement 复测）必须让路且本会话内主要在 yield。**未干预 Track-F、未偷其限速预算、未看候选表现**。
- **§3 TickFlow 回填**：`fetch_full_history` 保持 count=10000 单请求 + 6.5s pacing（10/min）；**未回退旧分页**。链式后台作业（entitlement 复测→回填 fetch --priority-boards BSE,STAR --max-minutes 240，均 yield Track-F、per-symbol 断点、可 SIGTERM 取消、独立锁、published-close 钳制、无重复 worker）已启动并**正确 yield 于活跃 Track-F**；当前 staged 235（54 BSE + 24 STAR）。**剩 ~1,782 FETCHABLE_NOT_PROBED 待 Track-F 空闲后 @10/min ≈ 3h 后台完成（断点可续跑），本会话因 Track-F 占用 TickFlow 未能实时完成——绝对优先所致，非缺陷**。
- **§4 覆盖对账**：master 5888 / TickFlow 覆盖 / BSE 权威 329 / STAR 613 / staged / panel 对账；**2 个缺失近上市 BSE（920079/920117）→ master_supplemental_additions.parquet（权威 provenance，不改冻结 H-028 母表）**；**双身份守卫：master 内 0 个旧 8xxxxx 码 ⇒ 0 dual-identity collision**（旧码仅存于 bse_code_mapping 的 historical_symbol=BLOCKED）；0 拒绝身份（仅加 akshare 权威 BSE 名）。runtime/data/u0/universe_reconciliation.json。
- **§6 TickFlow 权限复测**：governed 脚本 u0_tickflow_entitlement.py（yield Track-F，最多等 50min 否则写 DEFERRED）；已知状态（H-032B 昨测）ex_factors=PermissionError（无除权因子权限）、batch=PermissionError（无批量权限）、限速 10/min；**ex_factors 不可用 ⇒ 公司行动 = ALTERNATIVE_SOURCE_REQUIRED，绝不伪造复权身份**。
- **§7 严格 PIT 元数据溯源（真实闭合）**：新建 u0_pit_metadata.py。**3 字段闭合（带 symbol/effective_start/end/available_at/source/source_timestamp/source_hash 全 provenance）**：price_limit_regimes（7,328 区间行，交易所文档规则确定性派生：主板 10%/ChiNext 2020-08-24 起 20%/STAR 20%/BSE 30%）、ipo_special_limit_intervals（listing_date+规则派生）、**delisting_intervals（akshare SH+SZ 退市名单，367 退市日，此前=0）**。3 字段仍阻塞：st_intervals（无权威批量历史区间源=BLOCKED）、suspension_intervals（无权威区间源；不用 bar-gap 推断=ALTERNATIVE_SOURCE_REQUIRED）、corporate_action_identity（ex_factors 无权限=ALTERNATIVE_SOURCE_REQUIRED）。**历史绝不由当前元数据推断、未知绝不 default-false**。
- **§8 条数据决定**：**U0_BAR_NOT_READY_COVERAGE**（identity+provider PASS、五板全在列、coverage/quality 待回填）。**§10 严格 PIT 决定**：**FULL_UNIVERSE_DATA_NOT_READY_PIT**（阻塞字段 **4→3**：st/suspension/corp-action；delisting 已闭合）。**§11 训练门=严格 PIT；未 READY ⇒ 正式训练禁；条数据仅解锁 smoke**。
- **§13 剩余阻塞与精确源需求**：(a) 回填完成（TickFlow count=10000，Track-F 空闲后 ~3h）；(b) ST 历史区间（需交易所公告/名称变更历史，无 akshare 批量源）；(c) 停牌区间（需交易所停复牌历史）；(d) 公司行动身份（需 TickFlow ex_factors 权限开通 或 akshare 复权因子物化）；(e) 全宇宙 panel 加 is_suspended 旗标以分类 null-close。
- **Track F/S4 保全**：影子未改；S4 冻结哈希未变→不重跑。新增 3 governed 命令（→15 总）：source-u0-pit-metadata、audit-tickflow-entitlement（网络门控）、report-u0-reconciliation。VNext governance 面新增 PIT 闭合/退市日/对账/双身份显示。
- **测试**：backend 132 pass + 1 skip（entitlement 审计 skip=Track-F 后延未跑；h032c 6：delisting 闭合/provenance 列/PIT 反映闭合/双身份守卫/仅加权威/entitlement 契约）；frontend 36（governance 5 含 delisting 367+price_limit 断言）；tsc 干净；vite build OK；compileall OK；git diff --check 干净。资源：akshare PIT 溯源 ~6 req；无 TickFlow 主动占用（让路 Track-F）；RSS ≤4G；无 GPU；新磁盘 ~数 MB（PIT 区间表 + 报告）。改动文件：scripts/{u0_pit_metadata,u0_reconcile_universe,u0_tickflow_entitlement,u0_pit_readiness}.py、services/quant_api/services/{jobs,governance}.py、apps/quant-ui/src/vnext/governance/{GovernancePage.tsx,GovernancePage.test.tsx}、tests/{quant_ui/test_h031_governed,test_h032c_pit_closure}.py。

### H-032C 续跑（continue repair u0，2026-07-24）

- **§3 回填运行**：以 count=10000 单请求路径 + 6.5s（10/min）+ 全优先序（BSE→STAR→ChiNext→SH→SZ）后台启动；**全程正确让路 Track-F**——本会话 Track-F 正跑大规模 catch-up（catchup_supervisor+catchup_panel_chunked，实测 staged 1750/3487、~2.7h 且未完），持绝对优先且独占 TickFlow 10/min 预算，故 U0 回填正确 yield、本会话条覆盖**无法推进**（设计正确行为，非缺陷）。回填为有界后台/断点/可取消/让路作业，Track-F 空闲后自行推进；剩 ~1,784 票 ÷ 10/min ≈ ~3h。
- **§4 补充上市真正接入（修复半成品）**：先前 reconcile 只把 920079/920117 写入 master_supplemental_additions.parquet"留待未来重建"，**无任何消费者**。本续跑加 `load_master()`（backfill fetch+assemble）+ 等价 union（provider_coverage、build_security_master），去重且冻结 master 优先 ⇒ 2 只补充 BSE **真正进入回填 todo（1782→1784）与装配/覆盖矩阵/身份 master（master 5888→5890、BSE 327→329）**，零重复身份。
- **决定复算（含补充）**：BAR=U0_BAR_NOT_READY_COVERAGE（identity+provider PASS、五板全在列、coverage/quality 待回填）；PIT=FULL_UNIVERSE_DATA_NOT_READY_PIT（blocked=st/suspension/corp-action；delisting/IPO/price-limit 已溯源闭合）。训练门=PIT ⇒ 正式训练禁；条就绪仅解锁 smoke。Track F 3/7 未改；S4 哈希未变→不重跑。
- **测试**：backend 132 pass + 1 skip（新增 h032c 2：补充上市真进回填 master、union 去重冻结优先）；compileall OK；git diff --check 干净。改动文件：scripts/{u0_full_universe_backfill,u0_provider_coverage,u0_build_security_master}.py、tests/test_h032c_pit_closure.py。
