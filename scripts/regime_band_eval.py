...
Protocol:
  ...
  * Hold-band rows remain SIGNAL-DATED. The strict A-share simulator owns the
    only T-close -> next-session-close execution mapping.
  ...
  * Honest accounting: the grid winner on the full window is in-sample;
    ...
    BOTH halves of the window (H1/H2 robustness).

Any historical report produced by the previous ``delay_days=1`` + strict
simulator combination used a double delay and must not be used as promotion
evidence. Re-run this script after this fix before citing regime-band metrics.
...
def _cfg(name: str) -> HoldBandConfig:
    e, x, n = BAND[name]
    # Strict simulator owns T -> T+1. Never pre-delay this matrix.
    return HoldBandConfig(n_hold=n, entry_rank=e, exit_rank=x, delay_days=0)
...
def _half_metrics(nav: pd.Series, bench_daily: pd.Series, split: pd.Timestamp) -> dict:
    ...
    for tag, mask in (
        ("full", pd.Series(True, index=rets.index)),
        ("H1", rets.index <= split),
        ("H2", rets.index > split),
    ):
        r = rets[mask]
...
def _assert_signal_dated(weights: pd.DataFrame) -> None:
    semantics = weights.attrs.get("target_index_semantics")
    if semantics != SIGNAL_DATE_SEMANTICS:
        raise RuntimeError(
            "regime strict evaluation requires signal-dated hold-band targets; "
            f"got {semantics!r}"
        )
...
def main() -> int:
    ...
    panel = panel[
        (panel["trade_date"] >= start - pd.Timedelta(days=130))
    ...
    for bull, side, bear in itertools.product(GRID["bull"], GRID["sideways"], GRID["bear"]):
        ...
        rows.append(
            {
                "config": f"bull={bull}|side={side}|bear={bear}",
                "bull": bull,
                "sideways": side,
                "bear": bear,
                **{f"{k}_{kk}": vv for k, v in m.items() for kk, vv in v.items()},
        ...
        print(
            f"{bull[:12]}/{side[:12]}/{bear[:12]:14} full {m['full']['excess_ann']:+.2%} "
            f"H1 {m.get('H1', {}).get('excess_ann', float('nan')):+.2%} "
            f"H2 {m.get('H2', {}).get('excess_ann', float('nan')):+.2%} "
            f"turn {ts['mean_daily_turnover']:.2%}",
            flush=True,
    ...
    verdict = {
        ...
        "timing_warning": (
            "reports generated before the signal-date fix with hold-band delay_days=1 "
            "plus strict simulation are double-delayed and invalid for promotion"
        ),
