...
public BaoStock history.  It materialises PIT-safe registered factors, builds
next-session executable labels, and writes factor validity/lifecycle evidence.
...
from quantagent.factors.executable_labels import build_executable_forward_returns
...
RESEARCH_CYCLE_SCHEMA = "factor_research_cycle_v1"
...
def _research_baostock(
    *,
    ...
    # Raw/unadjusted bars avoid silently treating today's back-adjusted history
    # as point-in-time production evidence. Corporate-action handling remains a
    # separate research/data-governance concern.
...
def _materialise_factors(
    ...
    for name in names:
        ...
        except Exception as exc:  # research loop records failures; it never fabricates values
...
def run_cycle(args: argparse.Namespace) -> dict[str, object]:
    ...
    labeled = build_executable_forward_returns(market, horizons=horizons)
    ...
    # Keep a reproducible evidence snapshot, but no lifecycle mutation/promotion.
