...
## H-028 冻结策略定稿 + 全宇宙修复 + 执行冲击 + 盲化前向评估（状态：**TESTED — NOT_READY_REPRODUCIBILITY（前向保真 0.846<0.99 = FRESH 首读关键路径）+ INCOMPLETE_HISTORICAL_UNIVERSE（STAR/BSE 整板缺失新发现）；冻结书复现 PASS；√冲击交付、容量收紧至 ~1-10M；首读权威日期 2026-11-11；EXP-028，2026-07-13**）

- **性质**：无候选搜索/无调参/无 blend/top-K/持有期/regime 阈值搜索/无新 alpha 模型（H-025..027 已关闭搜索线）。全部候选定义冻结于 19e05f4；本票 = FRESH 首读前的基础设施与非选择性审计。**N 保持 115**（EXP-024 诊断类先例）。
- **候选集（S0-S4，冻结，机读清单 → runtime/reports/h028/frozen_candidate_manifest.json）**：S0 等权 eligible 基准 / S1 生产 2-sleeve rank k10（likely_overfit，诚实并评）/ S2 L1_c3ema07_minhold10 / S3 L1+D1_regime w0.5 / S4 RW1_4state。无法精确复现 ⇒ fail-closed。
- **§8 终选规则已预登记**：configs/preregistered_evals.json（13 条 AND 门 + 8 级字典序 + NO_PRODUCTION_STRATEGY 出口）——**在读任何 FRESH 表现之前写定并提交**。
- **Track A 宇宙修复**：证券主表（listing/delisting/board/ST/type，akshare+tushare 双源）+ 2020 后新股 TickFlow 日线回填（bounded 后台，checkpoint/resume）+ 退市覆盖量化 + 逐日 eligible 宇宙重建 + PIT/survivorship 审计。**新股上市 ≤60 交易日默认 ineligible（此处预注册）**；ST 史不可得的新名按含入+旗标注记（近似显式声明）。修不完 ⇒ INCOMPLETE_HISTORICAL_UNIVERSE，固定 cohort 绝不称全宇宙。
- **Track B 执行冲击**：新增 `impact = eta * vol20 * sqrt(order_value / ADV20)`，**eta=1.0（base）/ 2.0（stressed）在观察任何候选结果前冻结于此**（Almgren/Grinold 平方根律文献常规量级）；实现为独立模块 + 分析层 opt-in，**不改可信评测器默认**（INC-E1 先例：默认变更需用户批准）；单元测试覆盖零量/低 ADV/缺波动/涨跌停阻塞/部分成交/超额订单；不得以候选收益调 eta。
- **Track C 非选择性历史严格审计**（诊断，不选赢家，不改配置）：U0 固定 cohort 上 S1-S4 × 成本 {8,15,25bps, sqrt-base, sqrt-stress} × 资金 {1M,10M,30M}（网格声明：成本轴全格于 1M；资金轴于 8bps+sqrt-base}；复用 EXP-024 机制扩展；S1/S4 接入 best-effort，接不上如实记录。U1（修复宇宙）仅在 Track A 完成时跑**基准/容量语境**版（候选书不变——预测只存在于 cohort 名上，如实声明）。
- **Track D 盲化前向纸面**：runtime/paper/fresh_blind/ 全树 + SHA-256 哈希链 append-only ledger + 运营健康可见/候选表现加密隐藏（Fernet，钥匙本机存储=程序性盲化，诚实局限声明：root 可解，纪律靠 tooling 拒读 + 访问留痕）+ 每日 runner（数据更新→新鲜度校验→特征→预测→权重→风控→模拟→哈希）。**关键前置探针：v8.9 前向推理保真验证**（forward_daily_inference --run-dir retrain_plus7，validate 窗**只用 pre-quarantine 重叠段（≤2025-08-29）**，P6 要求 ≥0.99 特征保真）；探针不过 ⇒ 候选打分步登记 PENDING 状态，批式读时打分（freeze manifest 已许可）为后备。
- **§7 首读资格**：以实际交易日历程序化计算第 120 个 FRESH 交易日（面板真实日历 + 交易所未来日历），输出 fresh_first_read_eligibility.json；禁日历日近似。
- **禁触**：FRESH 窗候选表现零读取（Track D 产物加密存储不读）；隔离窗零新评测；候选定义零修改。
- **资源**：CPU 为主；Track A 回填后台 ≤2.5h 网络 + 磁盘 ≤2GB；Track C ≤45min；validate 探针 ≤40min；RAM ≤48G；GPU 仅推理探针（VRAM «20G）。
- **决定态（本票只出基础设施决定）**：READY_FOR_BLINDED_FORWARD_EVALUATION / NOT_READY_UNIVERSE / NOT_READY_EXECUTION_MODEL / NOT_READY_REPRODUCIBILITY。

## H-029 冻结 v8.9 前向推理保真修复（状态：**TESTED — FORWARD_FIDELITY_PASS（composite 0.99903；根因=7 个 llm_* 因子定义文件指错代 → 整列 NaN → missing-mask token；修复=双定义文件合并），EXP-029，2026-07-14；11-11 首读维持**）

- **来源**：EXP-028 探针（composite spearman 0.846 < 0.99）。本票唯一目标 = 定位首个失真层、按"只恢复冻结语义"规则修复、重发保真证书。**禁止**：alpha 搜索/改模型参数/改因子定义/改宇宙/改组合规则/读 FRESH 候选表现/事后降门槛。
- **黄金对照窗（先验冻结）**：**2025-06-02..2025-08-29**（64 交易日，pre-quarantine；存量 sleeve 预测覆盖完整 216,281 行/sleeve；断言零读取 ≥2025-09-01 行；**不读任何前向收益/IC——纯分数保真，标签零接触**）。
- **真值锚**：①gold 数据集 plus7clean（sha 272e4736…，存有 checkpoint 训练时的**精确特征值**）②存量 predictions.parquet（sha 已录 frozen_candidate_manifest）③ensemble blend 0.30/0.45/0.25（ensemble_summary.json）。
- **层分解（Stage 2 设计）**：L0 行群体（gold 键 vs 前向键：missing/extra）→ L1 原始特征逐列（recompute vs gold 127 列：MAE/max/日中位 spearman）→ L2 归一化特征（同群体 vs 异群体 rank）→ L3 每 sleeve 分数（(a) checkpoint(gold 特征+训练群体) vs 存量=**推理层保真** (b) 前向端到端 vs 存量=**综合保真**）→ L4 composite blend。首个 daily-median-spearman <0.995 的层 = 首失真层。
- **Stage 3 定向测试**（机器可读证据入 layer_comparison.csv/专项 json）：归一化群体（全 cohort vs eligible 子集 vs 训练行群体）/宇宙 mask/idx 存值 vs 重算/复权一致性/量额单位/min_periods/shift 与 available_at lag/NaN±inf 处理/列序 dtype/eval 模式/checkpoint-config 绑定/方向/并列 rank/blend rank 实现。
- **修复规则**：只许恢复冻结语义（历史宇宙 mask/历史特征实现/历史数据版本/列序 dtype/checkpoint 绑定/历史 rank 语义/环境变量/容器化原环境）；禁止再训练/新因子/为更好 IC 选归一化/新权重/新 top-K/放松阈值。
- **通过标准（沿用 EXP-028 预注册阈值，不因结果改动）**：composite 与每 sleeve 日中位 spearman ≥ **0.99**（黄金窗全 64 日）；无系统性偏置未解释；无缺日；schema 零失配;零隔离访问；确定性重跑差 ≤1e-6（rank 相关差）。
- **决定态（恰一）**：FORWARD_FIDELITY_PASS / FORWARD_FIDELITY_FAIL_ORIGINAL_RUNTIME_RECOVERABLE / FORWARD_FIDELITY_FAIL_FROZEN_CANDIDATES_UNREPRODUCIBLE / INCONCLUSIVE_MISSING_ARTIFACTS。不可复现 ⇒ S1-S4 标记运营不可复现、11-11 首读取消、正式退役报告 + 新冻结新时钟。
- **PASS 后动作**：7 交易日运营影子测试 → 哈希/schema/决策校验 → 安装盲跑日程 → 健康-only 暴露 → 表现保持加密 → 禁提前解盲。
- **资源**：CPU + GPU 推理（VRAM «20G）；特征重算 ≤40min/轮；磁盘 ≤500MB；总墙钟 ≤3h。

## H-031 VNext 集成 / 影子证书闭合 / 全宇宙数据就绪（状态：**DONE — 运营+数据票，非研究**；2026-07-23）

- **范围铁律**：不做 alpha 研究/模型选择/因子挖掘/候选调参/回测排名/表现解盲；不改冻结 S1-S4 定义、不改 2026-11-11 Track-F FRESH 时钟、不改隔离规则、不重写 append-only ledger（仅追加）。
- **四轨严格分离**：Track I 分支/架构集成；Track F 冻结影子运营闭合；Track S4 冻结批量重放保全；Track U0 历史全宇宙数据就绪。
- **决定态（本票产出）**：Track F = **SHADOW_TEST_ACCUMULATING**（2/7 有效日，未提前签证书）；Track S4 = **S4_BATCH_REPLAY_READY**（复认，代码/trace 哈希未变）；Track U0 = **FULL_UNIVERSE_DATA_NOT_READY_COVERAGE**（次阻 PIT；STAR/BSE 未回填、ST/停牌/退市/公司行动 BLOCKED_BY_DATA）；Track I = 无冲突（H-030 已在 origin/main，可远程恢复）。
- **架构契约**：VNext → 受治理 API → allowlisted JobRunner → Runtime 产物/manifest；浏览器不执行 shell/不收凭据/不隐式全量下载/不直改 runtime/不实现因子模型组合风控逻辑。新增 7 受治理命令均有 allowlist id + typed schema + Runtime-only 输出验证 + 显式网络批准 + 可取消/进度/锁/安全恢复 + manifest + 无合成回填。
- **禁触**：FRESH 窗候选表现零读取；受治理面表现非披露（存在性/关卡级字段 only）；候选定义零修改；backfill 优先级永低于 Track-F 日常，遇 Track-F supervisor/runner/guard 活跃即让路。
- **数据就绪 5 态词汇**：FULL_UNIVERSE_DATA_READY / NOT_READY_COVERAGE / NOT_READY_PIT / NOT_READY_PROVIDER / NOT_READY_INTEGRATION；无 READY 则禁一切模型训练。优先级 INTEGRATION>PROVIDER>COVERAGE>PIT>READY。
- **H-032 前瞻（预注册草案，未武装）**：候选集恰四（U-B0 PIT 等权基准 / U-M0 LightGBM 生产因子 / U-M1 FT-Transformer 三 sleeve 生产因子 / U-M2 至多一个预注册风险调整组合变换）；不重开模型园/因子搜索；下一代盲测时钟仅在 U0 就绪 + 历史验证 + 候选冻结后启动，且**不复用当前 Track-F FRESH 期**。

## H-032A 基于 main 的 U0 覆盖与 PIT 就绪闭合（状态：**DONE — 数据+运营票，非训练**；2026-07-23）

- **范围**：仅数据就绪与运营连续性；不做全宇宙训练/模型选择/因子挖掘/组合优化/严格回测/候选表现检视。从最新 main 起（H-031 已经 PR #19 合入），不重并旧分支。
- **单一数据决定 = FULL_UNIVERSE_DATA_NOT_READY_COVERAGE**（无 READY 则禁训练）。绑定证据：(a) BSE 覆盖 0 且 master 身份不完整（仅 920xxx、0 个 8xxxxx，无权威代码迁移表）= BLOCKED_BY_DATA；(b) 936 STAR/BSE FETCHABLE_NOT_PROBED 回填 backlog；(c) bar-panel 门 eligible-null-close + suspended-representation FAIL（绑定 suspension BLOCKED_BY_DATA）；(d) PIT ST/停牌/退市日/公司行动 BLOCKED_BY_DATA；(e) 退市生存者偏差量化（132/358 无行情、0 有退市日）。
- **关键诊断（§6 探针预承诺：先代表性探针再整板回填）**：STAR = FETCHABLE_NOT_PROBED（供应商可服务，纯回填缺口）；BSE 920x = FETCHABLE，8xxxxx = 供应商不服务 + master 缺失 ⇒ 身份 BLOCKED_BY_DATA。**空响应绝不当作"无历史"证据**（strict 词汇 NOT_PROBED/RETRYABLE_FAILURE/EMPTY_PROVIDER_RESPONSE/UNSUPPORTED_ENTITLEMENT/NO_RELIABLE_HISTORY/BLOCKED_BY_DATA）。
- **Track F/S4 保全**：影子 3/7 ACCUMULATING（未提前签证）；S4 三哈希未变 ⇒ 不重跑，S4_BATCH_REPLAY_READY 保全。
- **禁触**：候选表现零读取；FRESH 时钟/冻结定义/隔离规则不改；backfill 有界后台、绝对让路 Track-F、可 JobRunner 取消（SIGTERM 已实测优雅取消+锁释放）；无未管理 tmux 重复 worker。
- **H-032 交付前置（未满足，故未冻结/未许可训练）**：READY 后方冻结 U0 dataset/eligible-universe/labels/folds/embargo/候选矩阵；下一代盲测钟另起，不复用当前 FRESH。

## H-032B TickFlow 原生 U0 条数据与 PIT 闭合（状态：**DONE — 数据+运营票，非训练**；2026-07-23）

- **两独立决定**：**条数据就绪 = U0_BAR_NOT_READY_COVERAGE**（identity/provider PASS；五板全在列；coverage/quality 待回填完成）；**严格 PIT 就绪 = FULL_UNIVERSE_DATA_NOT_READY_PIT**（ST/停牌/退市/公司行动 BLOCKED）。训练门 = 严格 PIT；未 READY ⇒ 正式训练禁，条数据 READY 仅解锁 smoke。
- **TickFlow 保持主行情源不变**；其他权威源仅用于身份/历史状态/交易所规则/TickFlow 明确不提供的字段（akshare BSE 列表 = 身份；ex_factors 无权限 ⇒ 公司行动需替代源）。
- **决定性根因**：旧 ~100-bar 是**缺 count 参数**（SDK 默认 100），非服务器截断；`count=10000` 单请求返回全历史 ⇒ ~60x 提速。batch 无权限。实测限速 10/min。
- **BSE 身份纠正**：920xxx 是权威当前代码（BSE 已迁移，akshare 329 全 920xxx、0 个 8xxxxx，0 占位），非合成；H-032A 过度分类已更正为 BSE_IDENTITY_CURRENT_RESOLVED。空 8xxxxx 响应≠TickFlow 无 BSE 覆盖。
- **禁触**：候选表现零读取；S1-S4/FRESH 钟/影子注册表/加密台账/冻结推理运行时不改；backfill 有界后台、绝对让路 Track-F、可 JobRunner 取消；无未管理重复 worker；PIT 历史区间绝不由当前元数据推断、不 default-false。
- **§14 剩余时长**：剩 ~1,782 票 ÷ 10/min ≈ ~3h 有界后台。

## H-032C U0 TickFlow 回填完成与严格 PIT 闭合（状态：**DONE（回填后台续跑）— 数据+运营票，非训练**；2026-07-24）

- **两决定**：条数据 = **U0_BAR_NOT_READY_COVERAGE**（identity/provider PASS，五板全在列，coverage/quality 待回填）；严格 PIT = **FULL_UNIVERSE_DATA_NOT_READY_PIT**（阻塞 4→3：st/suspension/corp-action；delisting 已闭合）。训练门=严格 PIT；未 READY ⇒ 训练禁、仅 smoke。
- **Track-F 绝对优先**：会话内 Track-F catchup（3h timeout）占用 TickFlow，回填链式后台作业**正确 yield**，本会话内未实时完成回填（绝对优先所致）；机制正确、断点可续、后台 ~3h 完成。**未干预 Track-F、未偷预算、未看候选表现**。
- **TickFlow 保持主行情源**；PIT 元数据用权威非行情源：price_limit/IPO=交易所规则确定性派生，delisting=akshare 退市名单（367 日），ST/停牌/公司行动仍需替代源（ex_factors 无权限）。历史绝不由当前元数据推断、未知绝不 default-false。
- **§13 精确源需求**：回填完成 / ST 历史区间（交易所名称变更史）/ 停牌区间（交易所停复牌史）/ 公司行动（TickFlow ex_factors 权限 或 akshare 复权因子）/ 全宇宙 panel 加 is_suspended。
- **禁触**：S1-S4/FRESH 钟/影子/加密台账/S4 replay 不改；backfill 有界后台、绝对让路、可取消、无重复 worker；不回退旧分页；不伪造复权身份。
