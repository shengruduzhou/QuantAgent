...
下面链路从真实 market panel 开始，生成 labels、factors、gold training dataset、walk-forward OOS predictions、`target_weights` 和完整 paper report。你可以先用 `ridge` sanity，再改 `--model ft_transformer` 或通过 `optimize-alpha-v7` 做搜索。
